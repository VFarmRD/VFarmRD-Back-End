...
public interface PhaseRepository extends JpaRepository<Phase, Integer> {
    ...
    @Query(value = "SELECT p.phase_id FROM phases p where p.formula_id = :formula_id order by p.phase_id desc limit 1;", nativeQuery = true)
    int getNewestPhase_idOfFormula(@Param("formula_id") int formula_id);
